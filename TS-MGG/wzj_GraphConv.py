...
class GraphConv(nn.Module):
    ...
    norm : str, optional
        ...
        * ``both`` (default), where the messages are scaled with :math:`1/c_{ji}` above, equivalent
          to symmetric normalization. 等价于对称标准化

        * ``left``, to divide the messages sent out from each node by its out-degrees,
          equivalent to random walk normalization. 将每个节点发送的消息按出度划分，相当于随机遍历归一化。
    ...
    def forward(self, graph, feat, weight=None, edge_weight=None):
        ...
        * Weight shape: :math:`(\text{in_feats}, \text{out_feats})`.
        ...
        with graph.local_scope():
            ...
            feat_src, feat_dst = expand_as_pair(feat, graph)  # 源节点和目标节点
            ...
            if self._in_feats > self._out_feats:
                ...
                graph.srcdata['h'] = feat_src  # 源节点和目标节点
