...
def evaluate(config, model, val_x, val_y, val_length, test=False):
    ...
    with torch.no_grad():  # torch.no_grad()是一个上下文管理器，被该语句 wrap 起来的部分将不会track 梯度。
        for x_batch, y_batch, length_batch in batch_val:
            ...
            block_Dis = dgl.to_block(frontier_Dis, batch_nodes[0:-1])
            frontier_Dis_2 = dgl.in_subgraph(config.g_word_word_Dis, block_Dis.srcdata['_ID'])
            block_Dis_2 = dgl.to_block(frontier_Dis_2, block_Dis.srcdata['_ID'])
            block_Dis = block_Dis.to(config.device)
            block_Dis_2 = block_Dis_2.to(config.device)

            frontier_Pmi = dgl.in_subgraph(config.g_word_word_Pmi, batch_nodes[0:-1])
            block_Pmi = dgl.to_block(frontier_Pmi, batch_nodes[0:-1])
            frontier_Pmi_2 = dgl.in_subgraph(config.g_word_word_Pmi, block_Pmi.srcdata['_ID'])
            block_Pmi_2 = dgl.to_block(frontier_Pmi_2, block_Pmi.srcdata['_ID'])
            block_Pmi = block_Pmi.to(config.device)
            block_Pmi_2 = block_Pmi_2.to(config.device)

            frontier_Top = dgl.in_subgraph(config.g_word_word_Top, batch_nodes[0:-1])
            block_Top = dgl.to_block(frontier_Top, batch_nodes[0:-1])
            frontier_Top_2 = dgl.in_subgraph(config.g_word_word_Top, block_Top.srcdata['_ID'])
            block_Top_2 = dgl.to_block(frontier_Top_2, block_Top.srcdata['_ID'])
            block_Top = block_Top.to(config.device)
            block_Top_2 = block_Top_2.to(config.device)

            for i in range(batch_nodes.size()[0]):
                x_batch[x_batch == batch_nodes[i]] = i
            x_batch = x_batch.to(config.device)

            doc_represents, outputs = model((block_Dis, block_Pmi, block_Top), (block_Dis_2, block_Pmi_2, block_Top_2), x_batch, length_batch)
...
def train(config, model):
    ...
    real_train_x, val_x, test_x = \
        word_documents_total[real_train_mask], word_documents_total[val_mask], word_documents_total[test_mask]
    real_train_y, val_y, test_y = \
        labels_total[real_train_mask], labels_total[val_mask], labels_total[test_mask]
    real_train_length, val_length, test_length = \
        length_id_total[real_train_mask], length_id_total[val_mask], length_id_total[test_mask]
        ...
        for x_batch, y_batch, length_batch in batch_train:
            ...
            if total_batch % 1 == 0:
                ...
                msg = 'Iter: {0:>6},  Train Loss: {1:>5.2},  Train Acc: {2:>6.2%},' \
                      '  Val Loss: {3:>5.2},  Val Acc: {4:>6.2%}, Test Loss: {5:>5.2},' \
                      '  Test Acc: {6:>6.2%},  ({7:>6.2%})  Time: {8} {9}'
                print(msg.format(total_batch, loss.item(), train_acc, dev_loss, dev_acc, test_loss, test_acc,
                                 best_test_acc, time_dif, improve))
            total_batch += 1
...
def test(config, model):
    ...
    real_train_x, val_x, test_x = word_documents_total[real_train_mask], word_documents_total[val_mask], \
    word_documents_total[test_mask]
    real_train_y, val_y, test_y = labels_total[real_train_mask], labels_total[val_mask], labels_total[test_mask]
    real_train_length, val_length, test_length = length_id_total[real_train_mask], length_id_total[val_mask], \
    length_id_total[test_mask]
    ...
    test_acc, test_loss, test_report, test_confusion = \
        evaluate(config, model, test_x, test_y, test_length, test=True)
...
def get_represent_doc(config, model):
    print("Loading test data...")
    word_documents_train, y_train, length_id_train = process_dataset_sequence(config, 'train')
    train_indexs = np.zeros(y_train.shape[0])
    ...
    doc_represents = np.empty((0, 300), dtype=np.float32)
    # doc_represents = np.array([], shape=(0, 300), dtype=np.float32)

    batch_val = batch_iter(test_x, test_y, test_length)
    with torch.no_grad():  # torch.no_grad()是一个上下文管理器，被该语句 wrap 起来的部分将不会track 梯度。
        for x_batch, y_batch, length_batch in batch_val:
            batch_nodes = torch.unique(x_batch)
            y_batch = y_batch.to(config.device)
            ...
            doc_represent, outputs = model((block_Dis, block_Pmi, block_Top), x_batch, length_batch)
            loss = F.cross_entropy(outputs, y_batch)
            loss_total += loss
            ...
            doc_represents = np.append(doc_represents, doc_represent.cpu().numpy(), axis=0)

    acc = metrics.accuracy_score(labels_all, predict_all)

    msg = 'Test Loss: {0:>5.2},  Test Acc: {1:>6.2%}'
    print(msg.format(loss_total/len(val_x), acc))

    doc_represents_path = config.text_representation_save_path + str(acc) + ".npz"
    np.savez(doc_represents_path, arr1=labels_all, arr2=doc_represents)
...
if __name__ == '__main__':
    try:
        ...
        for i in range(len(each_word_out_degree)):
            ...
            # get_represent_doc(config, model)
